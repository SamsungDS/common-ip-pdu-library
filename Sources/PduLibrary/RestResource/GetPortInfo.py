...
class GetPortInfo(Resource):
    ...
    @swagger.operation(
        notes='API to fetch the metadata of Port',
        nickname='get_port_info',
        parameters=[
            {
                'name': 'body',
                'description': "API to fetch the metadata of a specified Port",
                'required': False,
                'allowMultiple': False,
                'dataType': GetPortInfoModel.__name__,
                'paramType': 'body'
            }
        ],
        responseMessage=[
    ...
    def post(self):
        ...
        try:
            ...
            response = self._pdu_library_manager.get_port_info(args.manufacturer,
                                                               args.ip,
                                                               args.username,
                                                               args.password,
                                                               args.port)
